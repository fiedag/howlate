    ...
    public static function get_requested_instance()
    {
        ...
        if (isset($_SESSION['xcrud_session'][$inst_name]['key']) && $_SESSION['xcrud_session'][$inst_name]['key'] == $key)
        {
            ...
            self::error('<strong>The verification key is out of date</strong><br />
                This mean that your browser cached previous version of this page with old key (in security reason verification key is generates every request)<br />
                Why? Maybe you was pressed back arrow in your browser or opened bookmark from last session. <strong>Just reload a page, nothing happened :)</strong>');
    }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ($mode == 'view')
                        {
                            ...
                            {
                                $this->result_row[$field] = str_ireplace('{value}', $this->result_row[$field], $this->column_pattern[$field]);
                                $this->result_row[$field] = $this->replace_text_variables($this->result_row[$field], $this->result_row, true);
                            }
            ...
            switch ($this->field_type[$field])
            {
                ...
                case 'file':
                    if ($value)
                    {
                        ...
                        $out .= isset($this->upload_config[$field]['text']) ? $this->upload_config[$field]['text'] : isset($this->upload_config[$field]['blob']) &&
                            $this->upload_config[$field]['blob'] ? 'blob-storage' : $value;
                        $out .= $this->close_tag('a');
                        break;
                    }
                case 'image':
        ...
        if (isset($this->field_type[$field]))
        {
            switch ($this->field_type[$field])
            {
                ...
                case 'checkboxes':
                    if (isset($this->field_attr[$field]['values'][$value]) && is_array($this->field_attr[$field]['values']))
                    {
                        $out .= $this->field_attr[$field]['values'][$value];
                    }
                    else
                    {
                        $out .= $value;
        ...
        if ($fields)
        {
            ...
            if (is_array($fields))
            {
                foreach ($fields as $key => $val)
                {
                    if (is_int($key))
                    {
                        ...
                        {
                            $tmp = explode('.', $val);
                            $field_names[$this->prefix . $tmp[0] . '.' . $tmp[1]] = array('table' => $this->prefix . $tmp[0], 'field' => $tmp[1]);
                            unset($tmp);
                        }
                    }
                    else
                    {
                        ...
                        {
                            $tmp = explode('.', $key);
                            $field_names[$this->prefix . $tmp[0] . '.' . $tmp[1]] = array(
            ...
            else
            {
                ...
                foreach ($fields as $key => $val)
                {
                    ...
                    {
                        $tmp = explode('.', $val);
                        $field_names[$this->prefix . $tmp[0] . '.' . $this->prefix . $tmp[1]] = array('table' => $this->prefix . $tmp[0],
                                'field' => $tmp[1]);
                        unset($tmp);
                    }
                }
        ...
        switch ($attr['data-type'])
        {
            ...
            default:
                if (!isset($fieldlist['default']) or $field == $this->column)
                {
                    ...
                    $attr['type'] = 'text';
                    $attr['value'] = (!is_array($phrase) && $field == $this->column or !$this->column) ? $this->html_safe($phrase) : '';
    ...
    protected function render_message()
    {
        ...
        if ($this->message)
        {
            $tag = array(
                ...
                'value' => $this->html_safe($this->message['text']));
