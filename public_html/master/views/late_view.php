...
<html>
    <head>
        ...
        <script>
            ...
            function assembleParam() {
                var str = "";
                $("#latenesses span").each(function() {
                    str = str + ((str == "") ? "" : ",") + $(this).attr('id');
                });
                return str;
            }
            ...
            function refreshLates() {
                ...
                var url = "<?php echo "http://secure." . __DOMAIN . "/late/pins?pins="; ?>" + str;

                $.getJSON(url, function(result) {

                    $.each(result, function(i, field) {
                        var j = i.replace(".", "\\.");  // the dot needs escaping!
                        $("#" + j).html(field);
                    });
    ...
    <body class="lateness">
        ...
        <img alt="" id="sysLogo" class="system-logo" 
             title="<?php if (isset($usercookie)) {
    echo "User cookie $usercookie .";
} if (isset($orgidcookie)) {
    echo "  OrgID cookie $orgidcookie";
} ?>" 
