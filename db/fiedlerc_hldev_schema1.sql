...
-- PHP Version: 5.4.23
-- Export with phpMyAdmin Custom, then Add DROP TABLE / VIEW / PROCEDURE / FUNCTION / EVENT statement, then GO
-- Edit the resulting SQL file and change the DEFINER to be equal to the unix user (either howlate, fiedlerc or other)
...
CREATE DEFINER=`fiedlerc`@`localhost` FUNCTION `fnTempTest`(`inMinutes` INT, `inLateToNearest` INT, `inLatenessOffset` INT) RETURNS varchar(90) CHARSET latin1
begin
